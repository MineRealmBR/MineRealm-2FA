...
/**
 * Java implementation for the Time-based One-Time Password (TOTP) two factor
 * authentication algorithm. To get this to work you:
 * 
 * <ol>
 * <li>Use generateBase32Secret() to generate a secret key for a user.</li>
 * <li>Store the secret key in the database associated with the user
 * account.</li>
 * <li>Display the QR image URL returned by qrImageUrl(...) to the user.</li>
 * <li>User uses the image to load the secret key into his authenticator
 * application.</li>
 * </ol>
 * 
 * <p>
 ...
 * 
 * <ol>
 * <li>The user enters the number from the authenticator application into the
 * login form.</li>
 * <li>Read the secret associated with the user account from the database.</li>
 * <li>The server compares the user input with the output from
 ...
 * 
 * <p>
 * For more details about this magic algorithm, see:
...
public class TimeBasedOneTimePasswordUtil {
	 ...
	 * {@link SecureRandom}. Could be used to generate the QR image to be shared
	 * with the user. Other lengths should use {@link #generateBase32Secret(int)}.
	 */
	...
	/**
	 * Validate a given secret-number using the secret base-32 string. This allows
	 * you to set a window in milliseconds to account for people being close to the
	 * end of the time-step. For example, if windowMillis is 10000 then this method
	 * will check the authNumber against the generated number from 10 seconds before
	 ...
	 *                     and still match. This checks before and after the current
	...
	/**
	 * Similar to {@link #validateCurrentNumber(String, int, int)} except exposes
	 ...
	 * @param timeStepSeconds Time step in seconds. The default value is 30 seconds
	...
	/**
	 * Similar to {@link #generateCurrentNumberString(String)} but this returns a
